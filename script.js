...
formTarget.addEventListener("load", () => {
  ...
  status.textContent = `${submittedName}，登記已提交！如有合適旅程，我們會透過你提供的聯絡方式通知你。`;
...
form.addEventListener("submit", (event) => {
  ...
  if (firstInvalid) {
    status.textContent = "請填寫姓名及有效電郵地址。";
    status.classList.add("is-error");
    firstInvalid.focus();
    return;
  }

  submittedName = form.querySelector('[data-field="name"]').value.trim();
  status.textContent = "正在安全提交你的登記…";
  status.classList.remove("is-error");
  ...
  submissionTimeout = window.setTimeout(() => {
    if (!isSubmitting) return;
    status.textContent = "提交時間較長，請稍後再試，或使用下方的 Google 表格連結。";
    status.classList.add("is-error");
    submitButton.disabled = false;
    isSubmitting = false;
  }, 12000);
